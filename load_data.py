...
def load_sentiment140_data():
    file =  codecs.open(f'./data/training.1600000.processed.noemoticon.csv', 'r', encoding='utf-8',
                    errors='ignore')

    csv_file = csv.DictReader(file, fieldnames=['target', 'id', 'date', 'flag', 'user', 'text'])
...
def load_imdb_data():
    ...
    file =  codecs.open(f'./data/imdb_master.csv', 'r', encoding='utf-8',
                    errors='ignore')
        ...
        if ".json.gz" in file:
            with gzip.open(f'/data/{file}') as f:
                for row in f:
                    ...
                    if 'reviewText' in json_row and 'overall' in json_row and json_row['overall'] != 3.0:
                        data.append({
